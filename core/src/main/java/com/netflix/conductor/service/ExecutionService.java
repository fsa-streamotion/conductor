...
public class ExecutionService {
		...
		try {
			...
			for (String taskId : taskIds) {
				...
				if (task == null || task.getStatus().isTerminal()) {
					...
					logger.info("Removed taskId from the queue: {}, {}", queueName, taskId);
					continue;
				}
	...
	private int requeuePendingTasks(Workflow workflow, long threshold) {
		...
		for (Task pending : tasks) {
			...
			if (pending.getUpdateTime() < threshold) {
				logger.info("Requeuing Task: {} of taskType: {} in Workflow: {}", pending.getTaskId(), pending.getTaskType(), workflow.getWorkflowId());
	...
	public int requeuePendingTasks(String taskType) {
		...
		for (Task pending : tasks) {
			...
			logger.info("Requeuing Task: {} of taskType: {} in Workflow: {}", pending.getTaskId(), pending.getTaskType(), pending.getWorkflowInstanceId());
